...
contract sBondToEarn is ERC20Permit, Ownable {
    ...
    event LogRebase( uint256 indexed epoch, uint256 profit, uint256 rebase, uint256 index );
    ...
    constructor() ERC20("Staked BondToEarn", "sBTE", 9) ERC20Permit() {
    ...
    function rebase( uint256 profit_, uint epoch_ ) public onlyStakingContract() returns ( uint256 ) {
        ...
        if ( profit_ == 0 ) {
            ...
            emit LogRebase( epoch_, profit_ , 0, index() );
            return _totalSupply;
    ...
    function _storeRebase( uint previousCirculating_, uint profit_, uint epoch_ ) internal returns ( bool ) {
        ...
        emit LogSupply( epoch_, block.timestamp, _totalSupply );
        emit LogRebase( epoch_, profit_, rebasePercent, index() );
